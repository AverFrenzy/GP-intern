...
export const PartyContextProvider = ({ children }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const pricesPerGuest = calculateAmountPerGuest(orderPrices.pizzaPrice, orderPrices.colaPrice, participantsData.party, pizzaEaters.length);
      const totalPartyInfo = participantsData.party.map(person => {
        return {
          ...
          shareToPay: person.eatsPizza ? pricesPerGuest.amountPerGuestPizzaAndCola : pricesPerGuest.amountPerGuestCola
        };
      });
  ...
  const calculateTotalOrder = (pizzaInfo, colaInfo, currencyInfo) => {
    ...
    const pizzaPriceExchanged = calculateExchangedPrice(pizzaCurrencyType, pizzaPrice, currencyInfo);
  ...
  const calculateAmountPerGuest = (pizzaPrice, colaPrice, participantsInfo, eatersOfPizzaNumber) => {
